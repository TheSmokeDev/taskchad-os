...
### Persona Toolset Grants (issue #427)

`/persona grant|revoke [<persona>] <toolset>` is the typed, admin-gated surface over the #426 self-provisioning executor. The command surface never edits YAML: `.claude/chat/persona_grant_commands.py` parses server-side (command text never enters an LLM prompt), resolves the persona, and calls `personas.services.add_persona_toolset` / `remove_persona_toolset` inside `asyncio.to_thread`.

**The role gate trusts the adapter's stamp — it is no longer re-derived here.** `IncomingMessage.user_role` is the canonical role-ingress seam (issue #424): every remotely-reachable adapter stamps it, at ingress, from its own authenticated identity data — Telegram/Discord/Slack/WhatsApp compare the sender against their own operator allowlist, Buzz resolves a signature-verified pubkey through a per-pubkey role map, and the CLI stamps `admin` unconditionally because reaching it needs a shell on the box, which already carries write access to the same `config.yaml`. The field's default is fail-closed `"viewer"`, and a repo-wide static test (`test_ingress_role_seam.py`) fails the build if any producer of an `IncomingMessage` forgets to state a role explicitly. `resolve_operator_identity` used to re-derive the allowlist check itself, once per sibling ticket (#427/#428/#429); it now trusts the stamp directly and keeps only three checks that are genuinely its own: `source == "interactive"` exactly (a cron/tool/hook turn, or grant text recovered from a recalled document, cannot authorize even with an admin stamp — the CLI stamps `admin` unconditionally regardless of `source`); the platform is on a positive trust list (`telegram`/`discord`/`slack`/`whatsapp`/`buzz`/`cli` — `web` is deliberately excluded because the retired `ws_client.py` relay shares `Platform.WEB` with the live always-`viewer` adapter but resolves its own role from client-supplied JSON with no allowlist behind it); and, for WhatsApp/Slack specifically, that platform's own allowlist env var is actually configured (`WHATSAPP_ALLOWED_NUMBERS` / `CHAT_ALLOWED_USERS` — a belt-and-suspenders check per the #424 design note, since the stamp cannot legitimately be admin there otherwise).

Persona defaulting inside a persona channel: the Discord channel→persona binding first, then the profile this bot process runs as; an explicit persona argument always wins, and with neither the command asks rather than guesses. Every exit that reaches the #426 executor — granted, revoked, already-true, refused — writes an append-only ledger row under that persona's own `data/`, keyed to the TARGET persona (never the ambient profile); a persona name that fails shape-checking before the executor is called is refused without a ledger row, since the row's own path would be derived from that untrusted name. A grant whose config write lands but whose OWN outcome-row append then fails is reported back verbatim (no "refused:" prefix) — `ToolsetGrantAuditError.applied` distinguishes that torn-write case from an unaudited refusal where nothing changed. `personas.services.describe_grant_failure` is the one canonical `REASON_*` → operator-text mapping (issue #435) — every calling surface routes its refusal/audit-error/config-shape exceptions through it instead of inventing its own phrasing, and each ledger row's `audit_id` is unique per row (timestamp + persona + operation + outcome + toolset + a short nonce), not just per attempt. Operator/architecture manual: `docs/manual/features/persona-self-provisioning.md`.
