...
_VOICE_PREAMBLE = (
    ...
    "Lead with the plain-English bottom line or a quick TLDR, then explain "
    "only what helps. Translate technical structure into normal speech. "
    "Everything you say is spoken aloud, so no markdown, no bullet lists, "
    "no emoji, no code blocks. Never read formatting or syntax aloud: do not "
    "say slash, backslash, dash, hyphen, colon, semicolon, underscore, pipe, "
    "asterisk, hash, open paren, close paren, or backtick as punctuation. "
    "Do not recite command syntax, file paths, URLs, model identifiers, or "
    "raw field names unless owner explicitly asks for the exact text. Explain "
    "what a command does in plain English; when exact syntax matters, keep the "
    "spoken explanation clean and say the exact text is in the transcript. "
