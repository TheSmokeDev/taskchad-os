# PRP-E02A-capability-descriptor-schema: Capability Descriptor and Registry Core

**Status:** draft — requires WF2 review and source-specific hardening
**Primary POL anchors:** POL-CP-001..POL-CP-003, POL-CP-005
**Depends on:** E01 audit/evidence contracts

## Goal and context
Define runtime-enforced descriptors, deterministic aliases, side-effect-free registration/discovery, version collision rules, and availability state.
...
- `.claude/chat/commands.py`

## Desired structured artifacts and state
- **CapabilityDescriptor schema exactly covers spec §7.5.**
- **RegistrySnapshot has digest, descriptors, alias map, availability and typed exclusions.**
- **Duplicate alias/ID-version conflicts fail construction atomically.**

Stable reasons include: invalid_descriptor, duplicate_descriptor, alias_conflict, unavailable, not_applicable. Unknown exceptions map to a typed `internal_error` with redacted detail; they do not become success.
