# PRP-E01C-safe-apply: Atomic Policy-Gated Amendment Apply

**Status:** draft — requires WF2 review and source-specific hardening
**Primary POL anchors:** POL-AM-002..POL-AM-004, POL-PA-002..POL-PA-004, POL-PA-007; invariants 4-6 and 16
**Depends on:** E01A, E01B

## Goal and context
Unify validate, current-policy authorization, snapshot, audit prepare, atomic target publication, verification, and ledger finalize in one amendment service boundary.
...
- `.claude/scripts/tests/test_cognition_amendments.py`
- `.claude/scripts/tests/test_amendment_pipeline_idempotence.py`

## Desired structured artifacts and state
- **ApplyIntent and result bind proposal/evidence/policy/operation digests and before/intended-after hashes.**
- **State: approved → apply_pending → applied, or rejected/failed/reconciliation_required.**
- **Target lock plus ledger transaction prevents cooperative interleaving; retry converges.**

Stable reasons include: policy_denied, approval_required, stale_authorization, target_conflict, snapshot_failed, audit_prepare_failed, apply_failed, verify_failed. Unknown exceptions map to a typed `internal_error` with redacted detail; they do not become success.
