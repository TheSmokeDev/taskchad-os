# PRP-E03H-cabinet-adapter: Cabinet Participant Turn Adapter

**Status:** draft — requires WF2 review and source-specific hardening
**Primary POL anchors:** POL-RT-006, POL-ID-002, POL-PA-001
**Depends on:** E03E

## Goal and context
Run each Cabinet participant through PersonaTurnService and record room, speaker, persona and source-event provenance without granting room-wide authority.
...
- `.claude/scripts/integrations/cabinet_api.py`
- `.claude/scripts/tests/test_cabinet_profile_execution.py`
- `.claude/chat/engine.py`

## Desired structured artifacts and state
- **CabinetTurnBinding maps room/speaker/persona/source events and scoped context.**
- **Participant policy decisions are independent and correlation-linked.**
- **Untrusted participant output is not operator learning evidence.**

Stable reasons include: participant_unavailable, persona_scope_mismatch, policy_denied, provenance_missing. Unknown exceptions map to a typed `internal_error` with redacted detail; they do not become success.
