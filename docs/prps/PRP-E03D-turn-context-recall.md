# PRP-E03D-turn-context-recall: Provenance-Bound Turn Context and Recall

**Status:** draft — requires WF2 review and source-specific hardening
**Primary POL anchors:** POL-RT-002, POL-RT-005, POL-ME-002; invariant 9
**Depends on:** E03A, E03C

## Goal and context
Assemble per-turn messages, bounded recall and transient cognition without mutating the manifest; route recall through RecallService with provenance.
...
- `.claude/chat/cognition/working_memory.py`

## Desired structured artifacts and state
- **TurnContext carries recall IDs/digests, retrieval time, scope and projection version.**
- **Optional cognition failure is a typed degraded no-effect outcome.**
- **Private monologue cannot enter transcript/learning events.**

Stable reasons include: recall_degraded, cognition_degraded, provenance_missing, budget_exceeded, isolation_failure. Unknown exceptions map to a typed `internal_error` with redacted detail; they do not become success.
