# PRP-E02D-approval-grants: Digest-Bound Approval Grants

**Status:** draft — requires WF2 review and source-specific hardening
**Primary POL anchors:** POL-PA-002, POL-PA-003, POL-PA-007
**Depends on:** E02C

## Goal and context
Create scoped, expiring, revocable, bounded-use approvals bound to normalized operation digests; argument/target/state changes invalidate them.
...
- **ApprovalGrant schema from spec §7.6; no reusable boolean approval.**
- **Atomic reserve/consume records use count, expiry, revocation and constraints.**
- **Canonical operation serialization excludes plaintext credentials.**

Stable reasons include: approval_missing, approval_expired, approval_revoked, approval_consumed, digest_mismatch, constraint_mismatch. Unknown exceptions map to a typed `internal_error` with redacted detail; they do not become success.
