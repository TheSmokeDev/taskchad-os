# PRP-E02F-leases-gateway-cutover: Capability Leases, Gateway Projection, and Cutover

**Status:** draft — requires WF2 review and source-specific hardening
**Primary POL anchors:** POL-CP-006, POL-PA-005, POL-MG-003, POL-MG-008, POL-MG-012
**Depends on:** E02B-E02E

## Goal and context
Issue least-privilege child/plugin leases, project registry status read-only, enforce migrated paths, and mechanically prevent old independent policy/invocation writes.
...
- **CapabilityLease binds capabilities, resources, tenant/persona, expiry and revocation.**
- **Gateway projection reports registry truth and legacy/not-applicable status.**
- **Cutover manifest records divergence resolution and disabled old-write paths.**

Stable reasons include: lease_expired, lease_revoked, lease_scope_denied, legacy_path_disabled, projection_stale. Unknown exceptions map to a typed `internal_error` with redacted detail; they do not become success.
