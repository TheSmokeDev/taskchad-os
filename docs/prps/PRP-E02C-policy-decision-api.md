# PRP-E02C-policy-decision-api: Central Policy Decision API

**Status:** draft — requires WF2 review and source-specific hardening
**Primary POL anchors:** POL-PA-001, POL-PA-002, POL-PA-004..POL-PA-006
**Depends on:** E02A, E01A

## Goal and context
Introduce one typed PolicyEngine API and adapters for route, Cabinet, live safety, integrations, kill switches, extensions, tenant and mutation rules.
...
- `.claude/scripts/tests/test_tenant_route_policy.py`
- `.claude/scripts/tests/test_cabinet_tool_policy.py`
- `.claude/chat/browser_workflows.py`

## Desired structured artifacts and state
- **PolicyDecision exactly covers spec §7.6 with evidence and policy-chain version.**
- **Decision enum is allow/deny/require_approval/allow_with_constraints.**
- **Shadow comparison never weakens a legacy security deny.**

Stable reasons include: identity_unresolved, policy_denied, approval_required, constrained, policy_error, rule_divergence. Unknown exceptions map to a typed `internal_error` with redacted detail; they do not become success.
