# PRP-E01B-evidence-binding: Confined Amendment Evidence Binding

**Status:** draft — requires WF2 review and source-specific hardening
**Primary POL anchors:** POL-AM-001, POL-ID-004, POL-ME-005; invariants 3 and 8
**Depends on:** E01A audit contract

## Goal and context
Resolve autonomous amendment citations under allowlisted roots, reject symlink/traversal/oversize/unreadable evidence, and bind byte digests before evaluation.
...
- **EvidenceBinding: root-relative path, media type, size, SHA-256, read time and source provenance.**
- **Proposal carries an immutable ordered binding set and its aggregate digest.**
- **Evaluation rejects missing/mutated evidence and model-authored claims cannot become operator evidence.**

Stable reasons include: evidence_outside_root, evidence_symlink, evidence_missing, evidence_unreadable, evidence_too_large, evidence_digest_changed. Unknown exceptions map to a typed `internal_error` with redacted detail; they do not become success.
