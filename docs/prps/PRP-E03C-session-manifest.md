# PRP-E03C-session-manifest: Immutable Session Manifest Compiler

**Status:** draft — requires WF2 review and source-specific hardening
**Primary POL anchors:** POL-RT-002, POL-RT-003, POL-CP-003; invariant 12
**Depends on:** E02A, E02C, E03A, E03B

## Goal and context
Compile/hash identity, policy, provider, capability schemas and prompt-critical config at session creation; change only through linked transition.
...
- **SessionManifest schema exactly covers spec §7.2; per-turn recall is excluded.**
- **Compiler output includes ordered prefix and digest.**
- **Transition event links predecessor/new manifests and reason.**

Stable reasons include: identity_unavailable, policy_unavailable, capability_unavailable, compile_failed, transition_required. Unknown exceptions map to a typed `internal_error` with redacted detail; they do not become success.
