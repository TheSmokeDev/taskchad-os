// Cron parsing/formatting — direct port from donor for the Scheduled
// page's visual builder + cron description. Pure-function utility, no
// network, no module-level mutable state.
...
export function buildSchedule(model: SchedModel): { cron: string; warning?: string } {
  ...
  if (minutes.length > 1 && hours.length > 1 && times.length !== minutes.length * hours.length) {
    ...
    warning = `Cron will also fire at ${extra} extra combination${extra === 1 ? '' : 's'}.`;
  }
